...
p_iso = iso.predict(x_grid.ravel())
woe_iso = np.log((p_iso + eps) / (1 - p_iso + eps))

# FastWoe WOE
encoder = FastWoe(binning_method="tree")  # faiss_kmeans
...
# ax1.plot(x_grid, woe_gmm, color=colors[2], label="GMM", linewidth=2)
ax1.step(
...
# ax1.plot(x_grid, woe_iso, color=colors[5], label="Isotonic", linewidth=2)
...
plt.savefig("images/woe_density.png", dpi=600)
plt.show()
